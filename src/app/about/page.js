    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <header className="relative w-full h-[450px]">
        <div className="relative w-full h-full">
          <Image src={aboutBanner} alt="About Us" fill className="object-cover brightness-75" />
        </div>
      ...
      <section className="py-16 bg-white px-6">
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-8 text-center">
          {coreValues.map(({ Icon, title, desc, color }, index) => (
            <div key={index} className="p-6 rounded-lg bg-gray-100 shadow-lg hover:shadow-xl transition-all duration-300">
              <div className={`mx-auto w-16 h-16 flex items-center justify-center rounded-full ${color} text-white`}>
                <Icon className="text-3xl" />
              </div>
              <h4 className="text-2xl font-semibold mt-5 text-gray-900">{title}</h4>
      ...
      <section className="py-16 px-6 text-center bg-gradient-to-b from-blue-50 to-white">
        <h3 className="text-4xl font-bold">
          Why Choose <span className="text-blue-600">Sky Scraper Skill?</span>
        </h3>
        <p className="text-lg text-gray-700 mt-4">
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-8">
          {whyChooseUs.map(({ Icon, title, desc, color }, index) => (
      ...
      <h3 className="text-4xl font-bold">Let&rsquo;s Build Something Amazing Together</h3>
